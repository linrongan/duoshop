...
class Curl{
    ...
    public function __construct($setopt=array())
    {
        ...
        curl_setopt($this->ch, CURLOPT_FOLLOWLOCATION, empty($this->setopt['follow_location'])?true:$this->setopt['follow_location']);
        //打开的支持SSL
        //if($this->setopt['ssl']){
            //不对认证证书来源的检查
        ...
        if($this->setopt['useCookie']){
            ...
            //连接关闭以后，存放cookie信息
            if (isset($setopt['cookiefile']))
            {
                curl_setopt($this->ch, CURLOPT_COOKIEJAR, $setopt['cookiefile']);
                curl_setopt($this->ch, CURLOPT_COOKIEFILE,$setopt['cookiefile']);
            }
            elseif (isset($_COOKIE['cookie']))
            {
                curl_setopt($this->ch, CURLOPT_COOKIEFILE,$_COOKIE['cookie']);
            }
            else{}
     ...
     * @return 错误返回:false 正确返回:结果内容
     */
    private function _request($method, $url, $params=array(), $uploadFile=array(), $referer='1')
    {
    ...
    {
        $fieldStr = $this->_parsmEncode($params,true);
        if($fieldStr){
     ...
     * @return string || array
     */
    private function _parsmEncode($params,$isRetStr=false)
    {
        $fieldStr = '';
        $spr = '';
        $result = array();
