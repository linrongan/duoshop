            ...
            <table class="table table-border table-bordered table-bg table-hover">
                ...
                <?php
                    if($data['refund_is_valid']==0)
                    {
                        ...
                        {
                            ?>
                            <td>退款确认<?php if($data['refund_type_id']==2 && $data['refund_status']==3){echo '(未填写退货信息)';} ?>：</td>
                            <td>
...
<script type="text/javascript">
    var page_url = '?mod=admin&v_mod=admin_refund&_index=_apply_details&id=<?php echo $_GET['id']; ?>';
    ...
    function reg_apply(obj,id)
    {
        ...
        }, function(index)
        {
            ...
            layer.close(index);
            request('post','?mod=admin&v_mod=admin_refund&_index=_apply_details&id='+id+'&_action=ActionRegApply',data,'reg_apply');
        });
    }
    ...
    function start_refund(obj,id)
    {
        ...
        layer.confirm('您已确定退款？(tip未退款仍然可以撤销)', {
            ...
            layer.close(index);
            request('post','?mod=admin&v_mod=admin_refund&_index=_apply_details&id='+id+'&_action=ActionStartRefund',data,'start_refund');
        });
    }
    ...
    function cpanel_order(obj,id)
    {
        ...
        layer.confirm('确定要重置退款？(tip重置后可重新申请)', {
            ...
            layer.close(index);
            request('post','?mod=admin&v_mod=admin_refund' +
                '&_index=_apply_details&id='+id+'&_action=ActionCpanelRefund',data,'cpanel');
        });
    }
</script>
</body>
