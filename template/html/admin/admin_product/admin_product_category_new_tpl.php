...
<article class="page-container">
    <form action="?mod=admin&v_mod=admin_product&_index=_category_new&_action=ActionNewCategory" method="post" class="form form-horizontal" id="form-admin-add">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3">分类图片(一级(450*230)其他(80*80))：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="hidden" name="category_img" value="" id="category_img">
                <div id="category_img_show"></div>
                    ...
                    if(!empty($category[0]))
                    {
                        foreach($category[0] as $item)
                        {
                            ...
                            while (!empty($category[$item['category_id']]))
                            {
                                ...
                                {
                                    ?>
                                    <option value="<?php echo $item['category_id']; ?>"><?php echo $str; ?><?php echo $item['category_name']; ?></option>
                                    <?php
                                }
                                $str.='____';
                            }
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input name="category_name" id="category_name" type="text" class="input-text"/>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span> 排序：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input name="ry_order" id="ry_order" type="text" class="input-text"/>
            </div>
        </div>
        <div class="row cl">
