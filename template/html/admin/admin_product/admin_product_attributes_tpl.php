...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20 mb-20"> <span class="l">
            <a href="javascript:;" onclick="attributes_add('添加属性','?mod=admin&v_mod=admin_product&_index=_attributes_new','','500')" class="btn btn-primary radius">
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        if($data['data'])
        {
            foreach($data['data'] as $item)
            {
                ...
                <tr class="text-c">
                    <td>
                        <?php echo $item['store_id']==0?'管理员':$item['store_name'];?>
                    </td>
                    ...
                    <td>
                        <a title="子属性" href="javascript:;"
                           onclick="add_child('子属性','?mod=admin&v_mod=admin_product&_index=_attributes_child','<?php echo $item['attr_type_id'];?>')" class="ml-5"
                           style="text-decoration:none"><i class="Hui-iconfont">&#xe600;</i>
                        </a>
                        <a title="编辑" href="javascript:;"
                           onclick="attributes_edit('属性编辑','?mod=admin&v_mod=admin_product&_index=_attributes_edit','<?php echo $item['attr_type_id'];?>','','500')" class="ml-5"
                           style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i>
                        </a>
...
<script type="text/javascript">
    ...
    function attributes_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: '?mod=admin&v_mod=admin_product&_index=_attributes&_action=ActionDelAttrType&id='+id,
                data:{'id':id},
                dataType: 'json',
