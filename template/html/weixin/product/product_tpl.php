...
<head>
    <meta charset="UTF-8">
    <title><?php echo isset($_GET['category_name'])?$_GET['category_name']:'全部产品'; ?></title>
...
<div class="mui-content">
    <form action="?mod=weixin&v_mod=product&_action=ActionGetSearch" method="post">
        <div class="web-classify-seach">
            <a href="?mod=weixin&v_mod=category&back=<?php echo urlencode('/?mod=weixin&v_mod=product'.$data['canshu'].$data['sort_canshu']); ?>" class="web-classify-seach-cage tc cl_b9 fl">
                <i class="fa fa-navicon"></i>
                <p class="f12 cl_b3">分类</p>
            </a>
            <div class="web-classify-seach-text fl">
                <div class="mui-input-row mui-search fr14">
                    <input type="search" name="keyword" value="<?php  echo isset($_REQUEST['keyword'])?$_REQUEST['keyword']:""?>" class="mui-input-clear" placeholder="请输入商品名称">
                </div>
            </div>
            <div class="cb"></div>
        </div>
...
<script>
    ...
    $(".apply-product-sort>a").click(function ()
    {
        ...
        if(canshu)
        {
            location.href='/?mod=weixin&v_mod=product<?php echo $data['category_canshu'].$data['canshu'];?>'+canshu;
        }
    });
