...
<div class="mui-content">
    <form action="/?mod=weixin&v_mod=group&_index=_product<?php echo (isset($_GET['category'])?'&category='.$_GET['category']:'').$data['canshu'].$data['sort_canshu']; ?>" method="post">
        <div class="web-classify-seach">
            <a href="?mod=weixin&v_mod=category&back=<?php echo urlencode('/?mod=weixin&v_mod=group&_index=_product'.$data['canshu'].$data['sort_canshu']); ?>" class="web-classify-seach-cage tc cl_b9 fl">
                        ...
                        <li class="web-yp-item">
                            <a href="?mod=weixin&v_mod=product&_index=_view&id=<?php echo $item['product_id']; ?>">
                                ...
                                <div class="web-yp-body">
                                    ...
                                    <div class="fr14 mtr5">
                                        <div class="fl red">
                                            ￥<span class="fr18"><?php echo $item['group_price']; ?></span>
                                        </div>
                                        <div class="fr cl_b9">
...
<script>
    ...
    $(".apply-product-sort>a").click(function ()
    {
        ...
        if(canshu)
        {
            location.href='/?mod=weixin&v_mod=group&_index=_product<?php echo $data['canshu'];?>'+canshu;
        }
    });
                ...
                $.getJSON('<?php echo $_SERVER['REQUEST_URI']; ?>&page='+page, function(res)
                {
                    layui.each(res.data, function(index, item)
                    {
                        lis.push('<li class="web-yp-item">'
                            ...
                            + '<div class="fr14 mtr5">'
                            + '<div class="fl red">'
                            + '￥<span class="fr18">'+item.group_price+'</span>'
                            + '</div>'
                            + '<div class="fr cl_b9">'
