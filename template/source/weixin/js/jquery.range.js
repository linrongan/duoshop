	...
	jRange.prototype = {
		...
		showPointerValue: function(pointer, position, animate) {
			...
			if ($.isFunction(this.options.format)) {
				...
				var zdtime = text*(24*60*60*1000);
				var alltime = (timedata.getTime() + zdtime) / 1000;
				$("#raiseDays").html(getLocalTime2(alltime));



/*
				//获取天数 1天 = 24小时  1小时= 60分 1分 = 60秒
				var oDay = parseInt(total / (24 * 60 * 60));

				//获取小时数
				//特别留意 %24 这是因为需要剔除掉整的天数;
				var oHours = parseInt(total / (60 * 60) % 24);

				//获取分钟数
				//同理剔除掉分钟数
				var oMinutes = parseInt(total / 60 % 60);

				//获取秒数
				//因为就是秒数  所以取得余数即可
				var oSeconds = parseInt(total % 60);

				//下面就是插入到页面事先准备容器即可;
				var html =  p(oHours) + " ：" + p(oMinutes)  + " : " + p(oSeconds);

				alert(html);
				/*
				document.getElementsByClassName('panic-pro-time')[0].innerHTML = html;
				*/
