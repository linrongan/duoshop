...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	{
        $xml = $this->createXml();
		$this->response = $this->postssl($xml,$this->url,$this->curl_timeout);
		return $this->response;
	}
...
class Refund_pub extends Wxpay_client_pub
{
    ...
    function SentRefund()
    {
        $refund_params = array(
            ...
            'out_refund_no'=>$this->parameters['out_refund_no'],
            'total_fee'=>$this->parameters['total_fee'],
            'refund_fee'=>$this->parameters['refund_fee'],
            'op_user_id'=>$this->parameters['op_user_id'],
        );
        $this->url = 'https://api.mch.weixin.qq.com/secapi/pay/refund';
		$refund_params['sign'] = $this->getSign($refund_params);
		$xml = $this->arrayToXml($refund_params);
		return $this->postXmlSSLCurl($xml,$this->url,30);
    }
}
