...
class user extends wx
{
    ...
    function GetOrderStatusCount()
    {
        ...
        $group_total = $this->GetDBSlave1()->queryrow("SELECT COUNT(*) AS group_count FROM "
            ." ".TABLE_GROUP." WHERE group_status=1 AND ".
            " end_time>'".date("Y-m-d H:i:s",time())."' AND userid='".SYS_USERID."'");
    ...
    function GetBankList()
    {
        $label = array();
        $bank = array();
        $data =  $this->GetDBSlave1()->queryrows("SELECT * FROM ".TABLE_BANK." WHERE "
            ." 1 ORDER BY bank_initial,bank_sort ASC");
        if($data)
        {
