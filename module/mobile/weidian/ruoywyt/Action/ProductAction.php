    ...
    function ActionAddCart()
    {
        ...
        if($attr)
        {
            ...
            foreach($this->data['attr_id'] as $k=>$val)
            {
                ...
                {
                    return array('code'=>1,'msg'=>'产品属性错误或已下架');
                }
                $attr_id .= $val.',';
            }
        ...
        $cart = $this->GetDBSlave1()->queryrow("SELECT * FROM ".TABLE_CART." "
            ." WHERE product_id='".$this->data['id']."' AND "
            ." userid='".SYS_USERID."' AND attr_id='".$attr_id."'");
        ...
        if($cart)
        {
            ...
            $res = $this->GetDBMaster()->query("UPDATE ".TABLE_CART." SET product_count='".$count."' "
                ." WHERE product_id='".$this->data['id']."' AND userid='".SYS_USERID."' "
                ." AND attr_id='".$attr_id."'");
        }else{
