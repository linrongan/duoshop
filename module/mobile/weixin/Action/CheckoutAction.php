...
class CheckoutAction extends checkout
{
    ...
    function ActionAddOrder()
    {
        ...
        foreach($data['cart']['cart'] as $key=>$item)
        {
            ...
            if (isset($this->data['discount_method']))
            {
                if ($this->data['discount_method']=="coupon")
                {
                    //优惠券使用减免程序
                ...
                }elseif($this->data['discount_method']=="discount")
                {
                    //计算招商折扣券金额抵扣
                    if ($order_pro_total>=100 && $data['user']['gift_balance']>0)
                    {
                        $discount_per=$this->GetDiscountCouponPer();
                        $gift_balance=intval($order_pro_total*$discount_per);
                        if ($data['user']['gift_balance']<$gift_balance)
                        {
                            $gift_balance=$data['user']['gift_balance'];
                        }
                        //减去折扣券金额
                        $this->GetDBMaster()->query("UPDATE ".TABLE_USER."  "
                            ." SET gift_balance=gift_balance-".$gift_balance." "
        ...
        $order_total = ($order_pro_total-$data['coupon']['use_coupon']['use_coupon_money']-$gift_balance)+$order_goods_fee;
        $last_id = $this->GetDBMaster()->insertquery("INSERT INTO ".TABLE_O_ORDER."(orderid,userid,"
            ...
            ." order_ship_address,order_img,order_addtime,order_status,liuyan,total_coupon_money,gift_balance)"
            ...
            ."'".$data['coupon']['use_coupon']['use_coupon_money']."','".$gift_balance."')");
        $remove_cart = $this->GetDBMaster()->query("DELETE FROM ".TABLE_CART."  "
            ." WHERE select_status=1 AND userid='".SYS_USERID."'");
    ...
    function ActionCheckPass()
    {
        ...
        if(empty($user['pay_password']))
        {
            redirect('/?mod=weixin&v_mod=user&_index=_info');
        }
        if(MD5($this->data['password'])!=$user['pay_password'])
        {
            die(json_encode(array('code'=>1,'msg'=>'支付密码错误')));
