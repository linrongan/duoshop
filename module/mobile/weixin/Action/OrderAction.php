...
class OrderAction extends order
{
    ...
    function ActionConfirmGetPro()
    {
        ...
        if(!empty($user['pay_password']))
        {
            ...
            {
                die(json_encode(array('code'=>1,'msg'=>'请输入支付密码')));
            }
            if(MD5($this->data['password'])!=$user['pay_password'])
            {
                die(json_encode(array('code'=>1,'msg'=>'支付密码错误')));
            }
        }
        if(!regExp::checkNULL($this->data['id']))
        {
    ...
    {
        if(!regExp::checkNULL($this->data['id']) || !regExp::checkNULL($this->data['select_address_id']) )
        {
            die(json_encode(array('code'=>1,'msg'=>'参数错误')));
        }
        $address = module('address')->GetOneAddress($this->data['select_address_id']);
        if(empty($address))
        {
            die(json_encode(array('code'=>1,'msg'=>'地址无效')));
        }
        $data = $this->GetDBSlave1()->queryrow("SELECT * FROM ".TABLE_O_ORDER_SHOP." "
            ." WHERE id='".$this->data['id']."' ");
        if(empty($data))
        {
            die(json_encode(array('code'=>1,'msg'=>'订单不存在')));
        }
        ...
        /*$this->GetDBMaster()->query("UPDATE ".TABLE_O_ORDER." SET "
            ." order_ship_name='".$address['shop_name']."',order_ship_phone='".$address['shop_phone']."', "
            ." order_ship_sheng='".$address_location[0]."',order_ship_shi='".$address_location[1]."', "
            ." order_ship_qu='".$address_location[2]."',order_ship_address='".$address['address_details']."' "
            ." WHERE orderid='".$data['orderid']."'");*/
        die(json_encode(array('code'=>0,
            'msg'=>'订单收货地址修改成功',
            'ship_name'=>$address['shop_name'],
            'ship_phone'=>$address['shop_phone'],
            'ship_province'=>$address_location[0],
            'ship_city'=>$address_location[1],
            'ship_area'=>$address_location[2],
            'ship_address'=>$address['address_details']
        )));
    }


    //重新退款
    function ActionResetRefund()
    {
        if(!regExp::checkNULL($this->data['goods_id']))
        {
            return array('code'=>1,'msg'=>'参数错误');
        }
        $data = $this->GetDBSlave1()->queryrow("SELECT * FROM ".TABLE_REFUND_APPLY." "
            ." WHERE goods_id='".$this->data['goods_id']."' AND "
            ." refund_userid='".$_SESSION['userid']."' AND refund_is_valid=0");
        if(!$data)
        {
            return array('code'=>1,'msg'=>'error');
        }
        $goods = $this->GetDBSlave1()->queryrow("SELECT * FROM ".TABLE_O_ORDER_GOODS." WHERE "
        ...
        $res1 = $this->GetDBMaster()->query("UPDATE ".TABLE_REFUND_APPLY." SET "
            ." refund_is_valid=1 WHERE goods_id='".$this->data['goods_id']."' "
            ." AND refund_userid='".$_SESSION['userid']."' AND refund_is_valid=0");
        $res2 = $this->GetDBMaster()->query("UPDATE ".TABLE_O_ORDER_GOODS." SET "
            ." goods_refund=0 WHERE id='".$this->data['goods_id']."'");
        if($res1 && $res2)
        {
            $this->GetDBMaster()->SubmitTransaction();
            return array('code'=>0,'msg'=>'ok');
