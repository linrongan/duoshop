...
class agent extends wx
{
    ...
    //获取代发产品的信息
    function GetOneAgentProduct($product_id)
    {
        return $this->GetDBSlave1()->queryrow("SELECT AP.id,AP.product_id,AP.agent_price,AP.agent_sort,AP.agent_sold,"
            ."P.product_name,P.product_img,P.product_price,P.store_id FROM ".TABLE_PRODUCT_AGENT." AS AP LEFT JOIN "
            ."".TABLE_PRODUCT." AS P ON AP.product_id=P.product_id AND P.product_status=0 "
            ."WHERE AP.product_id='".$product_id."' AND AP.agent_status=0");
    }
}