...
class point extends wx
{
    ...
    {
        $data=$this->GetDBSlave1()->queryrows("SELECT D.*,O.ship_status FROM ".TABLE_GIFT_ORDER_DETAIL." AS D "
            ." LEFT JOIN ".TABLE_GIFT_ORDER." AS O ON D.orderid=O.orderid "
            ." WHERE D.userid='".SYS_USERID."' "
            ." AND D.orderid='".$this->data['orderid']."'");
        return array("data"=>$data);
    }
    //获取积分日记列表
